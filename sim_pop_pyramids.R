...
hf_pat <-  read_csv("derived_data/sim_hf_dem.csv")
all_pat <- read_csv("derived_data/sim_all_dem.csv")
...
p1 <- ggplot(hf_pat2, aes(x = Age, y = Population, fill = Sex)) + 
  ...
  theme_bw()
ggsave("figures/sim_hf_age_sex.png",plot=p1)
p1
...
p2 <- ggplot(all_pat2, aes(x = Age, y = Population, fill = Sex)) + 
  ...
  scale_fill_brewer(palette = "Set1") + 
  theme_bw()
ggsave("figures/sim_all_age_sex_new.png",plot=p2)
p2
...
p3 <- ggplot(all_pat3, aes(x = Age, y = Population, fill = Sex)) + 
  ...
  scale_fill_brewer(palette = "Set1") + 
  theme_bw()
ggsave("figures/sim_all_age_sex.png",plot=p3)
p3
